...
class PrecompRegionDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenizer, opt, train):
        ...
        # 打印前5个图像特征向量 ------------------------------------------------ (5,36,2048)
        # print("First 5 image features:")
        # print(self.images[:5])
    ...
    def __getitem__(self, index):
        ...
        # caption_tokens = self.tokenizer.tokenize(caption) # 新版 -------------------- albert 改变这个6 + 
...
def process_caption(tokenizer, tokens, train=True):
    ...
    for i, token in enumerate(tokens):
        ...
        # sub_tokens = tokenizer.tokenize(token) # 新版 -------------------- albert 改变这个7 +
        ...
        if prob < 0.20 and train:  # mask/remove the tokens only during training
            ...
            # 10% randomly change token to random token
                ...
                for sub_token in sub_tokens:
                    ...
                    # output_tokens.append(random.choice(list(tokenizer.get_vocab().keys()))) # 新版 -------------------- albert 改变这个8 
