...
from lib.modules import l2norm, SelfAttention, Transformer, ExternalAttention, SimAM3D, ESAttention, SEAttention
...
class EncoderImagePrecomp(nn.Module):
    ...
    def forward(self, images, image_lengths):
        ...
        features = self.fc(images) # 变换特征的维度 自定义的 fc 函数已经定义好怎么变
...
class TextGNN(nn.Module):
    ...
    def __init__(self, input_dim=1024, hidden_dim=512, output_dim=1024, num_layers=2, dropout=0.1):
        ...
        # 第一层
        self.gnn_layers.append(GraphAttentionLayer(input_dim, hidden_dim, dropout=dropout))

        # 中间层
        for i in range(num_layers - 2):
            self.gnn_layers.append(GraphAttentionLayer(hidden_dim, hidden_dim, dropout=dropout))

        # 最后一层
        if num_layers > 1:
            self.gnn_layers.append(GraphAttentionLayer(hidden_dim, output_dim, dropout=dropout))
