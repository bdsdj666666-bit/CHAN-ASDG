    ...
    def __init__(self, opt):
        ...
        if opt.text_enc_type == 'bigru':
            ...
            if self.opt.optim == 'adam':
                ...
                bert_params = list(self.txt_enc.bert.parameters()) # ----------------------------------- 要改？
    ...
    def forward_emb(self, images, captions, lengths, image_lengths=None):
        ...
        if torch.cuda.is_available():
            ...
            if image_lengths is not None:
                if not isinstance(image_lengths, torch.Tensor):
                    image_lengths = torch.LongTensor(image_lengths).cuda()
                else:
                    # 如果已经是Tensor，确保它在正确的设备上
                    image_lengths = image_lengths.cuda()
            
            # 健壮地处理 lengths (caption lengths)
