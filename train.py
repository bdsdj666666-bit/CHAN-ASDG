...
# from torch.cuda.amp import autocast, GradScaler # 新增：导入混合精度训练所需模块
...
#         # tokenizer = BertTokenizer.from_pretrained(pretrained_model_name_or_path=root)  # -------------------- distilbert 改变这个4

#         tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-uncased')
        
#         # tokenizer = AlbertTokenizer.from_pretrained('albert-base-v2')
#         # vocab = tokenizer.get_vocab() # tokenizer.vocab # -------------------- albert 改变这个5 +
...
#     # construct model
#     scaler = GradScaler() # 新增：初始化GradScaler
#     model = Model(opt, scaler=scaler) # 修改：直接使用Model类实例化，并传递scaler
...
#         # train for one epoch
#         train(opt, train_loader, model, epoch, val_loader, scaler) # 修改：传递scaler对象
...
# def train(opt, train_loader, model, epoch, val_loader, scaler): # 修改：接收scaler对象
...
#         # 修改：使用autocast包装前向传播和损失计算，并传入scaler
#         with autocast():
#             model.train_emb(images, captions, lengths, image_lengths=img_lengths) # 修改：移除scaler参数
